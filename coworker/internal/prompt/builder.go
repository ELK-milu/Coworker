...
type PromptContext struct {
	...
	CoworkerMdPath     string         // COWORKER.md 路径
...
type BuildOptions struct {
	...
	IncludeCoworkerMd       bool // 包含 COWORKER.md
...
func (b *SystemPromptBuilder) Build(ctx *PromptContext, opts BuildOptions) string {
	...
	// COWORKER.md 内容
	if opts.IncludeCoworkerMd && ctx.CoworkerMdPath != "" {
		if claudeMd := b.loadCoworkerMd(ctx.CoworkerMdPath); claudeMd != "" {
...
- /help: Get help with using Coworker
...
// loadCoworkerMd 加载 COWORKER.md 文件
func (b *SystemPromptBuilder) loadCoworkerMd(path string) string {
...
// FindCoworkerMd 查找 COWORKER.md 文件
func FindCoworkerMd(workingDir string) string {
	// 查找顺序：
	// 1. 当前目录的 COWORKER.md
	// 2. .claude/COWORKER.md
	// 3. 用户目录的 ~/.claude/COWORKER.md

	candidates := []string{
		filepath.Join(workingDir, "COWORKER.md"),
		filepath.Join(workingDir, ".coworker", "COWORKER.md"),
	}

	// 添加用户目录
	if home, err := os.UserHomeDir(); err == nil {
		candidates = append(candidates, filepath.Join(home, ".coworker", "COWORKER.md"))
	}
